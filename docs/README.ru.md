...
- применять только исправления, которые не требуют придумывать смысловой текст;
- свернуть простые strict-only конструкции, если преобразование очевидно.

Исправления с шаблонами `TODO`, неоднозначные структурные преобразования и другие потенциально смысловые изменения не применяются командами `format` и `check --fix`; они могут оставаться отдельными явными действиями в редакторе. Настройки `ignore`, `select`, `# noqa` и `# sklint: disable/enable` применяются до выбора автофиксов.
