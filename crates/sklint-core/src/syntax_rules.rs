...
fn run_assignment_spacing(
    ...
    for line in lines {
        for (byte_idx, ch) in line.code.char_indices() {
            if ch != '='
                || line.text.as_bytes().get(byte_idx) != Some(&b'=')
                || is_assignment_exception(&line.code, byte_idx)
            {
                continue;
            }
                    ...
                    first_extra_item_after_top_level_comma(&line.code)
                {
                    ...
                    if let Some(replacement) = split_comma_items_preserving_indent(&line.text) {
                        diagnostic = diagnostic.with_fix(Fix {
                            safe: false,
                            message: "Split multiline bracket items (manual review required)"
                                .to_string(),
                            replacement,
                            start_line: line.no,
                            start_column: 1,
...
fn run_inline_temp_variable(
    ...
    for idx in 0..lines.len().saturating_sub(1) {
        ...
        let Some((name, _)) = simple_assignment(&line.code) else {
            continue;
        };
        let Some((raw_name, raw_expr)) = simple_assignment(&line.text) else {
            continue;
        };
        if raw_name != name {
            continue;
        }
        let expr = strip_inline_comment_raw(raw_expr).trim();
        if !is_safe_inline_expr(expr) {
            continue;
        }

        let statement_end_idx = logical_statement_end(lines, next_idx);
        let statement_text = lines[next_idx..=statement_end_idx]
            .iter()
            .map(|item| item.text.as_str())
            .collect::<Vec<_>>()
            .join("\n");
        let statement_code = lines[next_idx..=statement_end_idx]
            .iter()
        ...
        if next_code.starts_with("return ") || next_code.starts_with("yield ") {
            let inline_expr = inline_replacement(&statement_code, name, expr);
            let replacement = replace_word_once(&statement_text, name, &inline_expr);
            let statement_end = &lines[statement_end_idx];
            diagnostic = diagnostic.with_fix(Fix {
                safe: true,
                ...
                start_line: line.no,
                start_column: 1,
                end_line: statement_end.no,
                end_column: statement_end.text.chars().count() + 1,
            });
        }
        diagnostics.push(diagnostic);
    }
...
fn logical_statement_end(lines: &[LineInfo], start_idx: usize) -> usize {
    ...
    for (idx, line) in lines.iter().enumerate().skip(start_idx) {
        ...
        end_idx = idx;
        if depth == 0 && !line.code.trim_end().ends_with('\\') {
            break;
        }
    }
...
fn simple_assignment(code: &str) -> Option<(&str, &str)> {
    ...
    if !is_identifier_start(first) || !name_chars.all(is_identifier_continue) {
        return None;
    }
...
fn inline_replacement(statement_code: &str, name: &str, expr: &str) -> String {
    let expr = expr.trim();
    if is_direct_return_or_yield(statement_code, name) || is_primary_inline_expr(expr) {
        expr.to_string()
    } else {
        format!("({expr})")
    }
...
fn is_direct_return_or_yield(statement_code: &str, name: &str) -> bool {
    let statement = statement_code.trim();
    ["return ", "yield "].iter().any(|prefix| {
        statement
            .strip_prefix(prefix)
...
/// Return true only when `expr` is confidently a Python primary expression.
///
/// SK801 performs a textual substitution, so expressions with lower precedence
/// must be grouped whenever the temporary is used as part of a larger
/// expression. This deliberately conservative recognizer accepts identifiers,
/// literals, bracketed displays and their attribute/call/subscript chains. Any
/// expression that is not clearly primary is parenthesized before insertion.
fn is_primary_inline_expr(expr: &str) -> bool {
    let expr = expr.trim();
    let Some(mut cursor) = consume_primary_atom(expr, 0) else {
        return false;
    };

    loop {
        cursor = skip_ascii_whitespace(expr, cursor);
        if cursor == expr.len() {
            return true;
        }
        ...
        match ch {
            '.' => {
                cursor += ch.len_utf8();
                cursor = skip_ascii_whitespace(expr, cursor);
                let Some(next) = consume_identifier(expr, cursor) else {
                    return false;
                };
                cursor = next;
            }
            '(' | '[' => {
                let Some(next) = consume_balanced_group(expr, cursor) else {
                    return false;
                };
                cursor = next;
            }
...
fn consume_primary_atom(text: &str, start: usize) -> Option<usize> {
    let start = skip_ascii_whitespace(text, start);
    let ch = text[start..].chars().next()?;

    if matches!(ch, '(' | '[' | '{') {
        return consume_balanced_group(text, start);
    }
    if ch == '\'' || ch == '"' {
        return consume_string_literal(text, start);
    }
    if let Some(quote_start) = string_prefix_quote_start(text, start) {
        return consume_string_literal(text, quote_start);
    }
    ...
    {
        return consume_number_literal(text, start);
    }
    if is_identifier_start(ch) {
        return consume_identifier(text, start);
    }
    None
}

fn consume_identifier(text: &str, start: usize) -> Option<usize> {
...
fn string_prefix_quote_start(text: &str, start: usize) -> Option<usize> {
...
fn consume_string_literal(text: &str, quote_start: usize) -> Option<usize> {
    let quote = text[quote_start..].chars().next()?;
    if quote != '\'' && quote != '"' {
        return None;
    }
    let quote_len = quote.len_utf8();
    let triple = text[quote_start..].starts_with(&quote.to_string().repeat(3));
    let delimiter_len = if triple { quote_len * 3 } else { quote_len };
    let mut cursor = quote_start + delimiter_len;
    let mut escaped = false;

    while cursor < text.len() {
        if triple && text[cursor..].starts_with(&quote.to_string().repeat(3)) {
            return Some(cursor + delimiter_len);
        }
        let ch = text[cursor..].chars().next()?;
        cursor += ch.len_utf8();
        if escaped {
...
fn consume_number_literal(text: &str, start: usize) -> Option<usize> {
    let mut cursor = start;
    ...
    while cursor < text.len() {
        ...
        let allowed = ch.is_ascii_alphanumeric()
            || ch == '_'
            || ch == '.'
            || ((ch == '+' || ch == '-') && matches!(previous, Some('e' | 'E')));
        if !allowed {
            break;
        }
    ...
    saw_digit.then_some(cursor)
}

fn consume_balanced_group(text: &str, start: usize) -> Option<usize> {
    ...
    let mut cursor = start + opening.len_utf8();
    while cursor < text.len() {
        let ch = text[cursor..].chars().next()?;
        if ch == '\'' || ch == '"' {
            cursor = consume_string_literal(text, cursor)?;
            continue;
        }
        match ch {
            '(' | '[' | '{' => stack.push(ch),
...
fn strip_inline_comment_raw(text: &str) -> &str {
    let mut quote: Option<char> = None;
    let mut escape = false;
...
fn run_return_ternary(display_path: &str, lines: &[LineInfo], diagnostics: &mut Vec<Diagnostic>) {
    for idx in 0..lines.len().saturating_sub(2) {
        ...
        let raw_if = strip_inline_comment_raw(if_line.text.trim_start());
        let cond = raw_if
...
fn run_all_tuple(
    ...
    let (public, complete) = public_module_symbols(lines, defs);
    if public.is_empty() {
        return;
    }
    let all_line = lines
    ...
    if complete {
        ...
        let insert = if insert_after == 0 {
            format!("{}\n\n", tuple_all_line(&public))
        } else {
            format!("\n{}\n", tuple_all_line(&public))
        };
        let replacement = insert_text_after_line(source, insert_after, &insert);
        diagnostic = diagnostic.with_fix(Fix {
            safe: true,
            message: "Create complete __all__ tuple".to_string(),
            replacement,
            start_line: 1,
            start_column: 1,
            end_line: source_line_count(source),
...
fn public_module_symbols(lines: &[LineInfo], defs: &[DefInfo]) -> (Vec<String>, bool) {
    ...
    for line in lines {
        ...
        if trimmed.starts_with("from __future__ import ") {
            ...
            // Imports from typing modules are implementation-only type helpers
            // and must not become public exports or prevent a complete __all__
            // fix. Other public imports may be deliberate re-exports, which
            // cannot be decided safely without a Python resolver.
            if import_source_module(trimmed).is_some_and(is_type_support_module) {
                continue;
            }
            if trimmed.contains("import *") || unmatched_open_brackets(trimmed) > 0 {
                complete = false;
                continue;
            }
            if imported_names(trimmed).iter().any(|imported| {
                let alias_or_name = imported.split(" as ").last().unwrap_or(imported).trim();
                alias_or_name == "*"
                    || (!alias_or_name.starts_with('_') && !alias_or_name.is_empty())
            }) {
                complete = false;
            }
        } else if let Some(name) = public_assignment_name(trimmed) {
            symbols.insert(name.to_string());
        }
    }
    (symbols.into_iter().collect(), complete)
}

fn import_source_module(trimmed: &str) -> Option<&str> {
    let rest = trimmed.strip_prefix("from ")?;
    let (module, _) = rest.split_once(" import ")?;
    Some(module.trim())
}

fn is_type_support_module(module: &str) -> bool {
    matches!(module, "typing" | "typing_extensions")
}

fn public_assignment_name(trimmed: &str) -> Option<&str> {
...
fn unmatched_open_brackets(text: &str) -> usize {
    let mut depth = 0usize;
...
"#;
    ...
    #[test]
    fn sk801_uses_unicode_identifier_boundaries() {
        for source in [
            "# sklint: strict\n# sklint: ignore=SK804\ndef render():\n    x = get_value()\n    return xя\n",
            "# sklint: strict\n# sklint: ignore=SK804\ndef render():\n    x = get_value()\n    return яx\n",
            "# sklint: strict\n# sklint: ignore=SK804\ndef render():\n    x = get_value()\n    return x\u{301}\n",
            ...
            assert!(
                !codes(source, true).contains(&"SK801".to_string()),
                "SK801 must not treat a substring of a Unicode identifier as a use: {source}"
            );
        }
    }

    #[test]
    fn sk801_conditional_callable_fix_preserves_expression_precedence() {
        let vscode_config = VscodeConfig {
            strict: Some(true),
            ignore: vec!["SK804".to_string(), "SK309".to_string()],
            ..VscodeConfig::default()
        };
        let report = analyze(AnalysisInput {
            path: PathBuf::from("example.py"),
            source: "def build(condition: bool):\n    socket_cls = A if condition else B\n    return socket_cls(1)".to_string(),
            vscode_config,
        });
        let fix = report
            .diagnostics
            .iter()
            .find(|diagnostic| diagnostic.code == "SK801")
            .and_then(|diagnostic| diagnostic.fix.as_ref())
            .expect("SK801 safe fix exists");

        assert!(fix.safe);
        assert_eq!(fix.replacement, "    return (A if condition else B)(1)");
    }
...
",
    ...
    fn sk403_fix_is_not_bulk_safe() {
        ...
        let fix = report
            ...
            .iter()
            .find(|diag| diag.code == "SK403")
            .and_then(|diag| diag.fix.as_ref())
            .expect("SK403 fix exists");
        assert!(!fix.safe);
    }

    #[test]
    fn sk804_includes_public_final_constants_and_single_tuple_comma() {
        let vscode_config = VscodeConfig {
            strict: Some(true),
            ..VscodeConfig::default()
        };
        ...
        let fix = report
            ...
            .and_then(|diag| diag.fix.as_ref())
            .expect("SK804 fix exists");
        assert!(fix.replacement.contains("__all__ = (\"PUBLIC\",)"));
    }

    #[test]
    fn sk804_does_not_autofix_wildcard_imports() {
        let vscode_config = VscodeConfig {
            strict: Some(true),
            ..VscodeConfig::default()
        };
    ...
    #[test]
    fn sk804_inserts_before_first_statement_without_imports() {
        let vscode_config = VscodeConfig {
            strict: Some(true),
            ..VscodeConfig::default()
        };
        ...
        let replacement = &report
            ...
            .expect("SK804 fix exists")
            .replacement;
        assert!(replacement.starts_with("__all__ = (\"Public\",)\n\nclass Public:"));
    }
}
