        ...
        .filter(|(_, node)| node.kind == NodeKind::Class)
    {
        ...
        if methods.len() >= 2 {
            for pair in methods.windows(2) {
                let left = &nodes[pair[0]];
                let right = &nodes[pair[1]];
                // Spacing rules apply only to adjacent methods. Class fields,
                // assignments, nested classes, or any other statements between
                // methods must never be replaced by a blank-line autofix.
                if !gap_contains_only_blank_lines(
                    lines,
                    left.end_nonblank + 1,
                    right.group_start.saturating_sub(1),
                ) {
...
fn gap_contains_only_blank_lines(lines: &[String], start: usize, end: usize) -> bool {
    if end < start {
        return true;
    }
...
fn direct_methods(class_idx: usize, nodes: &[Node]) -> Vec<usize> {
    let direct_indent = nodes[class_idx].indent + 4;
    nodes
        .iter()
        .enumerate()
        .filter(|(_, node)| {
            node.kind == NodeKind::Function
                && node.parent == Some(class_idx)
                && node.indent == direct_indent
        })
        .map(|(idx, _)| idx)
        .collect()
}
...
fn run_nested_class_extra_blank_rule(
    ...
    let direct_method_indices = direct_methods(class_idx, nodes);
    let method_ranges: Vec<(usize, usize)> = direct_method_indices
        .windows(2)
        .map(|pair| (nodes[pair[0]].end_nonblank, nodes[pair[1]].group_start))
        .collect();
    let method_body_ranges: Vec<(usize, usize)> = direct_method_indices
        .iter()
        .map(|idx| (nodes[*idx].start, nodes[*idx].body_end))
        .collect();
    let string_ranges = multiline_string_ranges(lines, class.start + 1, class.body_end);
    ...
    while let Some(prev_line) = prev {
        ...
        if blanks > 0 {
            ...
            let inside_method_body = method_body_ranges
                .iter()
                .any(|(start, end)| *start <= prev_line && next_line <= *end);
            let inside_multiline_string = string_ranges
                .iter()
                .any(|(start, end)| *start <= prev_line && next_line <= *end);
            let after_initial_docstring =
                is_gap_after_initial_docstring(lines, class, prev_line, next_line);
            if !is_between_methods
                && !inside_method_body
                && !inside_multiline_string
                && !after_initial_docstring
            {
                push_blank_diagnostic(
                    diagnostics,
                    display_path,
...
fn multiline_string_ranges(
    ...
    for line_no in start_line..=end_line.min(lines.len()) {
        let line = &lines[line_no - 1];
        if let Some((quote, opening_line)) = open {
            if line.contains(quote) {
                ranges.push((opening_line, line_no));
                open = None;
            }
            continue;
        }
        let Some(quote) = opening_triple_quote(line) else {
            continue;
        };
        let Some(position) = line.find(quote) else {
            continue;
        };
        if !line[position + quote.len()..].contains(quote) {
            open = Some((quote, line_no));
        }
    }
    if let Some((_, opening_line)) = open {
        ranges.push((opening_line, end_line.min(lines.len())));
    }
    ranges
}
...
fn is_stub_function(function_idx: usize, nodes: &[Node], lines: &[String]) -> bool {
    ...
    let header_end = header_end_line(lines, function.start);
    if lines
        .get(header_end.saturating_sub(1))
        .map(|line| {
            strip_comment_and_strings(line)
                .trim_end()
                .ends_with(": ...")
        })
        .unwrap_or(false)
...
fn run_body_blank_rules(
    ...
    for (idx, node) in nodes.iter().enumerate() {
        ...
        // A one-line ellipsis overload/stub has no function body. Blank lines
        // after it belong to the surrounding class/conditional block and must
        // not be diagnosed as nested-function body spacing.
        if is_stub_function(idx, nodes, lines) {
            continue;
        }
        let parent_kind = node.parent.map(|parent| nodes[parent].kind);
        let nested_function = parent_kind == Some(NodeKind::Function);
        let string_ranges =
            multiline_string_ranges(lines, header_end_line(lines, node.start) + 1, node.body_end);
        if nested_function && config.is_enabled("SK302") {
            let child_ranges: Vec<(usize, usize)> = nodes
                .iter()
                .filter(|child| child.parent == Some(idx))
                .map(|child| (child.start, child.body_end))
                .collect();
            for (start, end, _len) in blank_runs_in_body(lines, node) {
                // Spacing inside a nested child definition is governed by that
                // child's own docstring/class/function rules. The outer nested
                // function must not remove blank lines required inside it.
                if child_ranges
                    .iter()
                    .any(|(child_start, child_end)| *child_start <= start && end <= *child_end)
                    || string_ranges.iter().any(|(string_start, string_end)| {
                        *string_start <= start && end <= *string_end
                    })
                    || is_blank_run_after_initial_docstring(lines, node, start, end)
                {
                    continue;
                }
                push_blank_run_diagnostic(
            ...
            for (start, end, len) in blank_runs_in_body(lines, node) {
                if string_ranges
                    .iter()
                    .any(|(string_start, string_end)| *string_start <= start && end <= *string_end)
                {
                    continue;
                }
                if len > 1 {
...
fn initial_docstring_close_line(lines: &[String], node: &Node) -> Option<usize> {
    let first =
        first_nonblank_in_range(lines, header_end_line(lines, node.start) + 1, node.body_end)?;
...
if TYPE_CHECKING:
    ...
    #[test]
    fn sk303_does_not_delete_class_fields_between_methods() {
    ...
    def __repr__(self) -> str:
        return "Message"
"#;
        let report = crate::formatter::format_source(
            std::path::PathBuf::from("example.py"),
            source.to_string(),
            crate::config::VscodeConfig::default(),
        );
        assert!(report.source.contains("first: int = 1"));
        assert!(report.source.contains("second: int = 2"));
    }
    #[test]
    fn nested_class_allows_blank_required_after_method_docstring() {
    ...
    return Inner"#;
        let found = codes(source);
        assert!(!found.contains(&"SK301".to_string()));
        assert!(!found.contains(&"SK613".to_string()));
    }

    #[test]
    fn conditional_inline_ellipsis_overloads_do_not_conflict_with_spacing_rules() {
...
class Factory:
    ...
    def __new__(cls, value: object) -> object:
        return value"#;
        let found = codes(source);
        assert!(!found.contains(&"SK302".to_string()));
        assert!(!found.contains(&"SK303".to_string()));
    }
    #[test]
    fn nested_function_docstring_spacing_is_not_removed_by_sk302() {
        let source = r#"def outer():
    ...
    return inner(1)"#;
        let found = codes(source);
        assert!(!found.contains(&"SK302".to_string()));
        assert!(!found.contains(&"SK615".to_string()));
        assert!(!found.contains(&"SK613".to_string()));
    }

    #[test]
    fn nested_class_method_separator_is_not_removed_by_extra_blank_rule() {
        let source = r#"def outer():
    class Inner:
        def first(self):
            return 1
