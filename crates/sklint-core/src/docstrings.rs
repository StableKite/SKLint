...
const FUNCTION_SECTION_ORDER: &[&str] = &["Args", "Returns", "Yields", "Raises"];
        ...
        for item in &doc.content {
            if let Some(name) = top_level_section_heading_name(doc, &item.text) {
                if !ALLOWED_SECTIONS.contains(&name.as_str()) {
                    diagnostics.push(doc_diag(
                        "SK606",
                        format!("Invalid docstring section `{name}`; only Args, Attributes, Returns, Yields and Raises are allowed"),
                        display_path,
...
{indent}{content}",
    ...
    if config.is_enabled("SK614") && description_lines_before_sections(doc).is_empty() {
        let mut diagnostic = doc_diag(
            "SK614",
            "Function, method and class docstrings must contain a description outside Args, Attributes, Returns, Yields and Raises",
            display_path,
            Span::new(doc.start_line, first_non_ws_col(lines[doc.start_line - 1]), doc.start_line, lines[doc.start_line - 1].chars().count() + 1),
            doc,
        );
        // A one-line docstring has no safe insertion point inside its quotes.
        // Keep the diagnostic, but do not offer a template edit that could be
        // inserted outside the string or overwrite the closing delimiter.
        if !doc.one_line {
            let insert_line = doc.start_line + 1;
    ...
    for pair in function_sections.windows(2) {
        ...
        for line_no in blank_lines {
            diagnostics.push(
                doc_diag(
                    "SK622",
                    "Args, Returns, Yields and Raises sections must not be separated by blank lines",
                    display_path,
...
fn run_function_section_order(
    ...
    diagnostics.push(
        doc_diag(
            "SK623",
            "Function and method sections must appear in Args, Returns, Yields, Raises order",
            display_path,
            Span::new(
                first.heading_line,
...
"#;
    ...
    #[test]
    fn sk614_does_not_offer_a_template_fix_for_one_line_docstrings() {
        let diagnostics = run_docstring_rules(
            Path::new("example.py"),
            "def values():\n    \"\"\"Args:\"\"\"\n    pass\n",
            &config(),
        );
    ...
    #[test]
    fn sk606_allows_yields_section() {
...
"#;
    ...
    #[test]
    fn sk605_allows_nominal_path_description() {
        let source = r#"def path() -> str:
    \"\"\"Путь файла\"\"\"
    return \"file.txt\"
"#;
        assert!(!codes(source).contains(&"SK605".to_string()));
    }
}
