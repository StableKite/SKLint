...
const fixes: Map<string, any> = new Map();
const suppressionLines: Map<string, number> = new Map();
const activeDiagnostics: Map<string, any[]> = new Map();
let extensionRoot = "";

const RU_DIAGNOSTICS: Record<string, string> = {
  ...
  SK306: 'Самостоятельные функции и классы верхнего уровня должны разделяться ровно тремя пустыми строками',
  ...
  SK505: 'Объявления должны располагаться выше первого eager-использования',
  ...
  SK507: 'raise ограничен lifecycle-контекстом; module __getattr__ может raise AttributeError',
  ...
  SK606: 'В докстринге разрешены только секции Args, Attributes, Returns, Yields и Raises',
  ...
  SK622: 'Между Args, Returns, Yields и Raises не должно быть пустых строк',
  SK623: 'Секции функций и методов должны идти в порядке Args, Returns, Yields, Raises',
...
function activate(context: any) {
  ...
  context.subscriptions.push(vscode.workspace.onDidOpenTextDocument((doc: any) => scheduleLint(doc)));
  context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc: any) => lintDocument(doc)));
  context.subscriptions.push(vscode.workspace.onDidCloseTextDocument((doc: any) => {
    collection.delete(doc.uri);
    activeDiagnostics.delete(doc.uri.toString());
  }));
  context.subscriptions.push(vscode.workspace.onDidChangeTextDocument((event: any) => {
    const runMode = vscode.workspace.getConfiguration('sklint', event.document.uri).get('run', 'onType');
    if (runMode === 'onType') {
      scheduleLint(event.document);
    }
  }));
  context.subscriptions.push(vscode.languages.onDidChangeDiagnostics((event: any) => {
    for (const uri of event.uris || []) {
      const doc = vscode.workspace.textDocuments.find((item: any) => item.uri.toString() === uri.toString());
  ...
  context.subscriptions.push(vscode.languages.registerCodeActionsProvider(
    ...
    new SKLintCodeActionProvider(),
    { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }
  ));

  context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider(
    { language: 'python', scheme: 'file' },
    new SKLintFormattingProvider()
  ));

  context.subscriptions.push(vscode.languages.registerHoverProvider(
    { language: 'python', scheme: 'file' },
    new SKLintHoverProvider()
  ));
...
function scheduleLint(document: any) {
...
function lintDocument(document: any) {
  if (!isPythonFile(document)) {
    return;
  }
  ...
  cp.execFile(command.executable, args, {
    ...
    if (error && error.code !== 1) {
      ...
      const diagnostic = new vscode.Diagnostic(
        range,
        `${SOURCE} ${l10n('failed to run', 'не запустился')}: ${stderr || error.message}`,
    ...
    try {
      ...
      const diagnostic = new vscode.Diagnostic(
        ...
        `${SOURCE} ${l10n('returned invalid JSON', 'вернул некорректный JSON')}: ${parseError.message}`,
        vscode.DiagnosticSeverity.Warning,
      );
      diagnostic.source = SOURCE;
      collection.set(document.uri, [diagnostic]);
    ...
    for (const item of payload.diagnostics || []) {
      ...
      const diagnostic = new vscode.Diagnostic(
        range,
        localizedDiagnosticMessage(item),
...
function clearDocumentCaches(uri: any) {
...
function shouldKeepDiagnostic(document: any, item: any, range: any, pyrightDiagnostics: any[], semanticRanges: any[]) {
  const code = String(item.code || '').toUpperCase();
  if (code !== 'SK701' && code !== 'SK702') {
    return true;
  }
  const config = vscode.workspace.getConfiguration('sklint', document.uri);
  if (config.get('pyrightAware.filterDiagnostics', true) && pyrightDiagnostics.some((diag: any) => rangesOverlap(range, diag.range))) {
    return false;
  }
  if (config.get('pyrightAware.requireMissingSemanticToken', true) && semanticRanges.length > 0) {
    return !semanticRanges.some((semantic: any) => rangesOverlap(range, semantic.range));
  }
  return true;
}

function pyrightLikeDiagnostics(uri: any) {
  return (vscode.languages.getDiagnostics(uri) || []).filter((diag: any) => {
    const source = String(diag.source || '').toLowerCase();
    return diag.source !== SOURCE && (source.includes('pyright') || source.includes('pylance'));
  });
}

async function semanticTokenRanges(document: any) {
  const config = vscode.workspace.getConfiguration('sklint', document.uri);
  ...
  try {
    ...
    const data = Array.from(tokens.data as number[]);
    const ranges = [];
    let line = 0;
    let character = 0;
...
function rangesOverlap(a: any, b: any) {
...
function suppressionCodeAtPosition(document: any, position: any) {
...
function lineDiagnosticsForCodeActions(document: any, range: any, context: any) {
  const seen = new Set<string>();
  const result = [];
  const addDiagnostic = (diagnostic: any) => {
...
function diagnosticIntersectsRequest(diagnostic: any, range: any) {
  if (!diagnostic || !range) {
    return false;
  }
  if (range.isEmpty) {
...
function ruleHelpHoverMarkdown(code: string, message?: string) {
...
function ruleHelpMarkdown(code: string) {
    ...
    for (const diagnostic of diagnostics) {
      ...
      if (fix) {
        ...
        if (!addedFixes.has(fixKey)) {
          ...
          action.edit.replace(
            document.uri,
            new vscode.Range(
              Math.max(0, fix.start_line - 1),
              Math.max(0, fix.start_column - 1),
              Math.max(0, fix.end_line - 1),
              Math.max(0, fix.end_column - 1),
            ),
            fix.replacement || '',
          );
          actions.push(action);
      ...
      if (!addedSuppressions.has(suppressKey)) {
        ...
        suppress.diagnostics = diagnostics
          .filter((item: any) => diagnosticCodeValue(item) === code)
          .filter((item: any) => (suppressionLines.get(diagnosticKey(document.uri, item)) ?? item.range.start.line) === suppressLine);
...
function dedupeActionsByTitle(actions: any[]) {
  const seen = new Set<string>();
  const result = [];
...
class SKLintFormattingProvider {
  async provideDocumentFormattingEdits(document: any) {
...
async function formatEdits(document: any) {
  const command = resolveSklintCommand(document.uri);
...
function execFileWithStdin(executable: string, args: string[], input: string, cwd: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = cp.execFile(executable, args, { cwd, timeout: 10_000, maxBuffer: 8 * 1024 * 1024 }, (error: any, stdout: string, stderr: string) => {
      if (error) {
...
function buildNoqaEdit(document: any, lineNumber: number, code: string) {
  ...
  if (noqa && noqa.index !== undefined) {
    ...
    if (codesText !== undefined) {
      const codes = codesText.split(',').map((part: string) => part.trim()).filter(Boolean);
      if (codes.map((c: string) => c.toUpperCase()).includes(code.toUpperCase())) {
        return undefined;
      }
...
function extractRuleSection(markdown: string, code: string) {
  const lines = markdown.split(/\r?\n/);
  const start = lines.findIndex((line: string) => line.startsWith(`## ${code} `) || line.startsWith(`## ${code} —`));
  if (start < 0) {
    return undefined;
  }
...
function isPythonFile(document: any) {
...
function resolveSklintCommand(uri: any) {
...
function resolveExecutable(uri: any) {
  const config = vscode.workspace.getConfiguration('sklint', uri);
  ...
  if (workspace) {
    candidates.push(
      ...
      path.join(workspace, '.venv', process.platform === 'win32' ? 'Scripts' : 'bin', exe),
      path.join(workspace, 'venv', process.platform === 'win32' ? 'Scripts' : 'bin', exe),
    );
  }
...
function resolveConfiguredExecutable(configured: string, uri: any) {
  if (!configured) {
    return undefined;
  }
...
function workspaceFolderFor(uri: any) {
  if (uri) {
...
function diagnosticCodeValue(diagnostic: any) {
...
function diagnosticSeverity(item: any) {
...
function stripTrailingSentencePeriod(text: string) {
